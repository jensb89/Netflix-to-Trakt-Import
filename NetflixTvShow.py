...
class NetflixTvHistory(object):
    ...
    def hasTvShow(self, showName):
        return self.getTvShow(showName) != None
    ...
    def addTvShowEntry(self, showName, seasonNumber, episodeTitle, watchedDate, seasonName = None ):
        ...
        logging.info("Added show %s season %d episode %s" % (show.name, int(season.number) if season.number is not None else -1, episode.name))
    ...
    def getJson(self):
        ...
        for show in self.shows:
            ...
            for season in show.seasons:
                jsonOut["tvshows"][show.name].append({'SeasonNumber' : season.number , "SeasonName" : season.name, 'episodes' : {} })
